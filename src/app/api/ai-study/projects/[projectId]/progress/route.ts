...
import { formatAiStudyError, getAiStudyProjectProgress } from "@/lib/ai-study";
...
export async function GET(_request: Request, context: RouteContext) {
  ...
  try {
    ...
    const generationProgress = await getAiStudyProjectProgress(user.id, projectId);
    return apiOk({ generationProgress });
  } catch (error) {
