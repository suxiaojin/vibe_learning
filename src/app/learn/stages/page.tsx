  ...
  return (
    <StudentPageShell active="learn" maxWidthClassName="max-w-3xl">
      <div style={getLearningPathThemeStyle(settings.learningPathTheme)}>
        <Link className="inline-flex items-center gap-2 text-sm font-semibold text-slate-500 transition hover:text-[var(--challenge-accent)]" href={group ? `/learn?course=${group.key}` : "/learn"}>
          <ArrowLeft size={22} />
          {text.back}
        </Link>
          ...
          <div className="space-y-5">
            {group?.courses.map((course) => (
              <details key={course.id} className="group space-y-3" open>
                <summary className="flex min-h-11 cursor-pointer list-none items-center gap-3 rounded px-1 transition-colors hover:bg-surface active:bg-surface-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--challenge-ring)] [&::-webkit-details-marker]:hidden">
                  <h1 className="flex min-w-0 flex-1 items-baseline text-2xl font-semibold text-ink">
                    <span className="shrink-0 text-sm text-[var(--challenge-accent)]">{text.course}</span>
                ...
                {course.chapters.map((chapter) => {
                  ...
                  return (
                    <section
                      ...
                      key={chapter.id}
                      className={`scroll-mt-4 overflow-hidden rounded-panel border border-border-soft/80 shadow-card ${current ? "bg-[var(--challenge-muted)]" : completed ? "bg-surface" : "bg-surface-muted"}`}
                    >
                      <div className="p-5">
                        <div>
                          <div className="flex items-start justify-between gap-4">
                            ...
                            {locked ? (
                              ...
                              <Link className="secondary-button shrink-0 px-3 hover:border-[var(--challenge-accent)] hover:text-[var(--challenge-accent)] focus-visible:ring-[var(--challenge-ring)]" href={`/learn/${chapter.sections[0].id}/guide`}>
                                <BookOpenText size={18} />
                                {text.guide}
                              </Link>
                            )}
                          ...
                          <div className="mt-5 flex items-center gap-3">
                            {locked ? <Lock className="shrink-0 text-slate-400" size={18} /> : <span className="grid size-6 shrink-0 place-items-center rounded-full bg-[var(--challenge-primary)] text-white">{completed ? <Trophy size={16} /> : <Sparkles size={16} />}</span>}
                            <div className="h-3 flex-1 rounded-full bg-slate-100">
                              <div className="h-3 rounded-full bg-[var(--challenge-primary)]" style={{ width: `${percent}%` }} />
                            </div>
                            <span className="w-10 text-right text-xs font-semibold text-slate-400">{percent}%</span>
                          </div>
                          <div className="mt-7">
                            {completed ? (
                              <Link className="secondary-button w-48 hover:border-[var(--challenge-accent)] hover:text-[var(--challenge-accent)] focus-visible:ring-[var(--challenge-ring)]" href={`/learn?course=${group.key}&chapter=${chapter.id}`}>
                                <RotateCcw size={18} />
                                {text.review}
                              </Link>
                            ) : current ? (
                              <Link className="success-button w-64 bg-[var(--challenge-primary)] hover:bg-[var(--challenge-strong)] focus-visible:ring-[var(--challenge-ring)]" href={`/learn?course=${group.key}&chapter=${chapter.id}`}>{text.current}</Link>
                            ) : (
