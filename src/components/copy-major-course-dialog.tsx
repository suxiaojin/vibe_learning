...
import { copyMajorCourse } from "@/app/admin/actions";
...
type CopyMajorCourseDialogProps = {
  chapterCount: number;
  courseId: string;
  courseName: string;
  majorId: string;
...
export function CopyMajorCourseDialog({
  chapterCount,
  courseId,
  courseName,
  majorId,
  ...
  return (
    <>
      <button
        ...
        title={availableRegions.length ? "复制到其他区域" : "其他区域已存在同名课程，或该专业尚未绑定其他区域"}
        type="button"
      >
      ...
      >
        <form action={copyMajorCourse}>
          ...
          <div className="grid gap-5 px-6 py-5">
            ...
            <div>
              <label className="label" htmlFor={`copy-target-${courseId}`}>目标区域</label>
              <select className="input rounded-none" defaultValue={availableRegions[0]?.id} id={`copy-target-${courseId}`} name="targetRegionId" required>
