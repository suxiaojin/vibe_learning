...
type AiStudyLearningProject = {
  ...
  tasks?: AiStudyLearningTask[];
};

type AiStudyProjectLearningViewProps = {
  backHref: string;
  backTitle: string;
  ...
  nodeHrefBase: string;
  nodes: NodeListItem[];
  project: AiStudyLearningProject;
  queryError?: string;
  requestedNodeId?: string;
  viewerUserId: string;
};

export async function AiStudyProjectLearningView({
  backHref,
  backTitle,
  headerAccessory,
  mode = "student",
  ...
  requestedNodeId,
  viewerUserId
}: AiStudyProjectLearningViewProps) {
  const tree = buildNodeTree(nodes);
  const orderedNodes = flattenTree(tree);
  const selectedNodeId = orderedNodes.find((node) => node.id === requestedNodeId)?.id || orderedNodes[0]?.id || "";
  ...
  const hasExplicitNode = Boolean(requestedNodeId && requestedNodeId === selectedNodeId);
  ...
  const nodeDetail = selectedNodeId ? await getAiStudyNodeDetail(viewerUserId, selectedNodeId) : null;
  ...
  const readOnly = mode === "admin-preview";
      ...
      <header className="sticky top-0 z-30 border-b border-[#e5e7eb] bg-[#f3f4f6] px-4 md:px-5">
        <div className="flex h-14 items-center justify-between gap-4">
          <div className="flex min-w-0 items-center gap-3">
            <Link className="grid size-8 shrink-0 place-items-center rounded-[8px] text-[#111827] transition hover:bg-white" href={backHref} title={backTitle}>
              <ArrowLeft size={19} />
            </Link>
            <div className="flex min-w-0 items-center gap-3">
        ...
        <section className="min-h-[calc(100dvh-112px)] min-w-0 overflow-hidden rounded-[16px] bg-white shadow-[0_12px_32px_rgba(16,24,40,0.05)]">
          <div className="flex h-14 items-center justify-between border-b border-[#edf0f4] bg-[linear-gradient(96deg,#fff7e8_0%,#fffdf8_42%,#f8fbff_100%)] px-5">
            ...
            <div className="flex items-center gap-2">
              {previousNode ? (
                <Link className="grid size-9 place-items-center rounded-full text-[#344054] transition hover:bg-[#f4f6f8]" href={buildNodeHref(nodeHrefBase, previousNode.id)} title="上一个知识点">
                  <ChevronLeft size={20} />
                </Link>
              ) : (
              ...
              {nextNode ? (
                <Link className="grid size-9 place-items-center rounded-full text-[#344054] transition hover:bg-[#f4f6f8]" href={buildNodeHref(nodeHrefBase, nextNode.id)} title="下一个知识点">
    ...
    <div className="rounded-[16px] border border-[#edf0f4] bg-[#fbfcfd] px-6 py-8">
      ...
      <div className="mt-7 grid gap-3 md:grid-cols-3">
        {(project.tasks || []).slice().reverse().map((task) => {
          const taskTypeLabel = taskTypeLabels[task.type as keyof typeof taskTypeLabels] || task.type;
          const taskStatusLabel = taskStatusLabels[task.status as keyof typeof taskStatusLabels] || task.status;
          return (
            <div key={task.id} className="rounded-[12px] border border-[#e5e9ef] bg-white px-4 py-3">
              <div className="flex items-center justify-between gap-2">
                <p className="text-sm font-black text-[#101828]">{taskTypeLabel}</p>
...
function getGenerationProgress(project: AiStudyLearningProject) {
